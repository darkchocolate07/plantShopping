...
const ShoppingCart = () => {
  ...
  const totalAmount = cartItems.reduce((total, item) => total + Number(item.cost.substring(1)) * item.quantity, 0);
  ...
  return (
    ...
    <div className="shopping-cart">
      ...
      <div className='Total-Amount'>{totalAmount ? <div>The total amount is {totalAmount}</div> : ''}</div>
        ...
        {cartItems.map(item => (
              <li key={item.id} className="cart-item">
                ...
                <div className='item-total'>
                  total={item.quantity* Number(item.cost.substring(1))}
                </div>
